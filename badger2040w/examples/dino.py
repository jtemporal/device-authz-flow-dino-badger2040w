"""
This modifies the Dino game found here https://github.com/niutech/dino-badger2040
to use device authz flow powered by Auth0 for login and score submission.
"""
import badger2040
import badger_os
import io
...
high_score_path = "/dino/highscore.txt"
high_score = 0
score = 0

dino_img = Image("/dino/dino.pbm")
cactus_img = Image("/dino/cactus.pbm")
...
def start_text():
    ...
    # display.text("Press B to upload game score", 20, 100, scale=2)
    display.set_update_speed(badger2040.UPDATE_TURBO)
    display.update()
...
def endgame_text():
    ...
    display.text("Press UP to start, A to abort", 20, 60, scale=2)
    display.text("High score: " + str(high_score), 20, 80, scale=2)
    display.text("Press B to upload score", 20, 100, scale=2)
    display.set_update_speed(badger2040.UPDATE_TURBO)
    display.update()


def device_authz_end_text():
    clear_screen()
    display.set_update_speed(badger2040.UPDATE_FAST)
    display.text("Dino Game", 20, 20, scale=2)
    display.set_font("bitmap8")
    display.text("Connecting to Auth Server", 20, 60, scale=2)
    display.text("High score: " + str(high_score), 20, 80, scale=2)
    display.text("", 20, 100, scale=2)
    display.set_update_speed(badger2040.UPDATE_TURBO)
    display.update()
...
def main():
    ...
    while True:
        ...
        if display.pressed(badger2040.BUTTON_UP):
            ...
            print("Requesting tokens in name of the user")
